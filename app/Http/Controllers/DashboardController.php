...
class DashboardController extends Controller
{
    ...
    public function index(): Renderable
    {
        ...
        $user_container = $helper->getContainerWhere();

        $percentage = $helper->percentageOfBin(
            end($user_container['tracking_data'])['current_depth'],
            $user_container['container_depth']
        );
        ...
        if ($percentage >= 85 && !isset($container_recommendation["error"])) {
            $recommended_data_helper = new Helper(
                Auth::user()->city_id,
                $container_recommendation["street_id"]
            );

            $recommended_remaining = $recommended_data_helper->containers[count($recommended_data_helper->containers) - 1]['current_depth'];

            try {
                $amount_of_times = $container_recommendation["container_depth"] / $recommended_remaining;
            ...
            $link = "https://www.google.nl/maps/dir/" .
                $user_container['latitude'] . "," .
                $user_container['longitude'] . "/" .
                $container_recommendation['latitude'] . "," .
                $container_recommendation['longitude'];

            $link_in_iframe = "https://www.google.com/maps/embed/v1/directions?key=AIzaSyA1s66kXMm6obk6K67NcL1zvTNwgAC7KTU&origin=" .
                $user_container['latitude'] . "," .
                $user_container['longitude'] . "&destination=" .
                $container_recommendation['latitude'] . "," .
                $container_recommendation['longitude'] . "&zoom=13";
        }
    ...
    public function realtime_chart(): JsonResponse
    {
        ...
        foreach ($helper->getAllStreetsWhere() as $label) {
